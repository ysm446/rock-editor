...
奥に Rock Bridge（未破断部）を残す。現在は未加工の Box 1個に対する、軸に沿った1回の切り込みに対応する。曲面やノイズ付きの母岩には、新しい「Mesh 全幅溝」を使う。
Box 専用方式は回転90度単位。貫通して Bridge が消える設定は診断する。
「Mesh 全幅溝」は曲面・任意角度に対応し、U 全幅と +V 側の外面を覆う範囲で、奥に未破断部を残す。
未加工の母岩1個・切り込み1回・4096三角形以下に限定。有限長や交差する複数亀裂は未対応。Bridge 断面の数値と緑の表示は Box 専用方式のみ。
