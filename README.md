...
曲面・ノイズ付き母岩も Fracture へ接続できる。初期実装は未分割の岩1個、単純な断面ループ1本、分割後の各側が1連結体の場合。穴のある断面・再帰分割・Joint Set による多片分割は未対応。
`Base Rock → Joint Set → Mesh Output` で、方向・間隔・位置/角度ばらつき・本数・Seed を持つ有限パッチ列を表示できる。
Joint Set を直列接続すると複数方向を重ねられる。設定は保存・Undo に対応する。現段階はガイド表示で、母岩の切断は行わない。
