...
bool ValidatePresetMaterial(const PresetMaterial& material, std::string& error) {
    ...
    if (material.mask) {
        ...
        if ((mask.shape != LayerMaskShape::Constant && mask.shape != LayerMaskShape::Noise) ||
            !range(mask.noiseScaleMeters, 0.05f, 100) ||
            ...
            !range(mask.strength, 0, 1)) return fail("レイヤーの被覆マスクが不正です");
    }
    return true;
}
...
uint32_t AddPresetNode(PresetGraph& graph, PresetNodeKind kind, std::array<float, 2> position) {
    ...
    if (kind == PresetNodeKind::Mask) { node.settings.mask.emplace(); node.settings.mask->shape = LayerMaskShape::Constant; }
...
bool CompilePresetMaterials(const LayerMaterial& preset, std::vector<PresetMaterial>& materials, std::string& error) {
    ...
    if (materials.empty() || materials.size() > 4) { error = "素材は1〜4層にしてください"; return false; }
    for (const auto& layer : materials) if (!ValidatePresetMaterial(layer, error)) return false;
    materials[0] = SourceMaterial(materials[0]);
