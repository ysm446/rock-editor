...
ShadowCascadeData BuildShadowCascades(const Camera& camera, const DirectX::XMFLOAT3& lightDirection,
    ...
    if (count == 1) {
        ...
        const float extent = radius * float(std::max(resolution, 8u)) / float(std::max(resolution, 8u) - 4);
    ...
    for (uint32_t cascade = 0; cascade < count; ++cascade) {
        ...
        size_t index = 0;
        for (float distance : {start, end}) for (float y : {-1.0f, 1.0f}) for (float x : {-1.0f, 1.0f}) {
            corners[index] = XMVector3TransformCoord(XMVectorSet(x * tanX * distance, y * tanY * distance, -distance, 1), inverseView);
            center = XMVectorAdd(center, corners[index++]);
        }
        center = XMVectorScale(center, 1.0f / 8);
        float extent = 0;
        for (const auto corner : corners) extent = std::max(extent, XMVectorGetX(XMVector3Length(XMVectorSubtract(corner, center))));
        ...
        XMFLOAT3 lightCenter; XMStoreFloat3(&lightCenter, XMVector3TransformCoord(center, lightView));
        ...
        const auto projection = XMMatrixOrthographicOffCenterRH(lightCenter.x - extent, lightCenter.x + extent,
            lightCenter.y - extent, lightCenter.y + extent, -radius, radius);
