...
void RecentFiles::Load(const fs::path& storage) {
    ...
    if (!stream) return;  // まだ履歴が無いだけ。エラーにしない。
    const auto document = json::parse(stream, nullptr, false);
    if (!document.is_object() || document.value("format", json()) != kFormat) return;
...
void RecentFiles::Add(const fs::path& root, const fs::path& scene) {
    ...
    AddRoot(root);
    Insert(m_roots.front().scenes, scene);
    Save();
}
...
void RecentFiles::Save() const {
    ...
    for (const auto& entry : m_roots)
        roots.push_back({{"path", ToUtf8Portable(entry.path)}, {"scenes", WritePaths(entry.scenes)}});
    const json document = {{"format", kFormat}, {"version", kVersion}, {"roots", roots}, {"projects", WritePaths(m_legacy)}};
