...
json WritePresetMaterial(const graph::PresetMaterial& material) {
    ...
    if (material.mask) {
        ...
        m["mask"] = {{"shape", static_cast<uint32_t>(mask.shape)},
            {"noiseScale", mask.noiseScaleMeters}, {"threshold", mask.threshold},
...
graph::PresetMaterial ReadPresetMaterial(Reader& r, const json& m, uint32_t version) {
    ...
    if (version >= 2) {
        ...
        if (!mask.is_null()) {
            graph::LayerMaskSettings settings;
            settings.shape = static_cast<graph::LayerMaskShape>(r.UInt(mask, "shape"));
            settings.noiseScaleMeters = r.Float(mask, "noiseScale");
...
bool ReadLayerMaterial(const json& body, graph::LayerMaterial& material, std::string& error) {
    ...
    if (layers.empty() || layers.size() > 4) { error = "素材は1〜4層にしてください"; return false; }
