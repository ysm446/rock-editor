...
[実装計画](../plan/plan.md) の各段階で、追加した実処理を呼ぶテストとビューポート確認を行う。P0～P3 では Debug ビルドと CTest、Release での Box・有限パッチ・軸に沿う部分切断の表示と保存復元を確認した。詳細と未検証事項は [進捗](../plan/progress.md) を参照。
