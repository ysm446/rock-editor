...
- 重いScatter/VoronoiとUVをキャッシュする。最大128片のSelect/Filter/Transformは軽量な走査と参照・行列の更新とし、分割を再実行しない。
...
初期限界案は1入力立体、2〜128点、伸長比の最大/最小比16以内、入力2万三角形、出力合計25万三角形。製品上限の確定値ではなく、実測後に見直す。上限超過時は部分結果を完成品として返さない。
...
- 現在のTo VolumeはBoxes入力専用なので、Piecesを直接SDFへ戻す接続は提供しない。任意Meshのボリューム化は独立した後続機能。
