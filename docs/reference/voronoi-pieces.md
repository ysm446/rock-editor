...
2026-09-26追記：Layered Boxes と Pieces 入力の個別分割、面内の点配置、Rim・層指定を追加した。現在の拡張仕様は [薄板の積層とピースの欠け](layered-pieces.md) を参照。以下は従来の単体メッシュ入力を中心とした設計記録。
...
- 重いScatter/VoronoiとUVをキャッシュする。最大1024片のSelect/Filter/Transformは軽量な走査と参照・行列の更新とし、分割を再実行しない。
...
- Voronoi Fracture の設定欄の「表示」に「ワイヤーフレーム」「ポイント」（どちらも既定オフ）。このノードを選んでいる間、どちらかをオンにすると岩の面の描画（影を含む）を止め（`PreviewRenderer::SetMeshSceneHidden`）、分割片の稜線（深度付きの線）と Points 入力の点（Scatter Points のプレビューと同じ丸）だけを表示する。表示用の状態で、ノードの設定・保存・Undo には含めない。稜線は `geometry::PieceEdges`：切断面を囲む辺と、元の外面のうち 20 度より大きく折れた辺（箱の角など）。切断面の三角形分割の対角線と、丸い外面の細かい三角形の辺は出さない。分割片と点は評価キャッシュから読むので、評価中は前回の結果を出す。
- Piece Filter の設定欄の「表示」に「除外した片」（既定オフ）。オンにすると、このノードで除外した分割片（入力の片のうち、出力に同じ ID で残らなかった片）を深度付きの稜線で重ねる。残った片は面のまま描画する（岩の面は隠さない）。重い計算のキャッシュ（`pieceEntries`）は Scatter / Voronoi だけなので、評価のたびにピース系ノードの直近の出力を `RockEvaluationCache::pieceOutputs` に控える（共有参照で複製しない。失敗したノードの分は消す）。Voronoi の稜線もここから読む。
...
限界は1入力立体、2〜1024点、伸長比の最大/最小比16以内、入力2万三角形、出力合計25万三角形。上限超過時は部分結果を完成品として返さない。

2026-09-26 に点数の上限を512から1024へ引き上げた。各片は近い点から順に切り、片の半径の2倍より遠い点で打ち切る（その二等分面は片を切らない）。切るたびに使われなくなった頂点を詰め、点数の2乗の切断面の表は持たない。結果の形は従来と同じ（三角形数が一致）。Release の実測（`std::execution::par`）：

| 入力 | 点数 | 変更前 | 変更後 |
|---|---|---|---|
| Box（12三角形） | 512 | 22 ms | 6〜8 ms |
| Box（12三角形） | 1024 | — | 12 ms |
| 凸の球（約1.2万三角形） | 512 | 435 ms | 211 ms |
| 凸の球（約1.2万三角形） | 1024 | — | 291 ms |

Piecesの再分割では、子のID `(親ID + 1) × 512 + 子の番号` の間隔 512（`PieceIdStride`）を変えない。保存済みの手動選択・個別変換がIDで片を指すためで、1片あたりの点数も512までとする（合計は1024まで）。
