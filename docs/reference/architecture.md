...
[v2 原仕様](../rock_generator_spec_v2.md) を既存実装へ接続するための設計案。4種類の母岩・弱い BaseNoise の CPU メッシュ・評価・描画変換と有限亀裂パッチのガイド表示は実装済み。軸に沿う単一 Box の部分切断と Bridge の計算・表示まで実装済み。単一平面の完全分割と2片の Chunk 操作も実装済み。部分切断の任意角度化と複数切断は今後の設計。工程は [実装計画](../plan/plan.md) を参照。
...
今回は有限パッチ列の表示基盤。パッチからの実分割、continuity/roughness と有限亀裂の接続は実装していない。パッチの重なり表示を亀裂の接続判定とは扱わない。
