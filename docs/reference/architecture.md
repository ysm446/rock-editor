...
[v2 原仕様](../rock_generator_spec_v2.md) を既存実装へ接続するための設計案。4種類の母岩・弱い BaseNoise の CPU メッシュ・評価・描画変換と有限亀裂パッチのガイド表示は実装済み。軸に沿う単一 Box の部分切断と Bridge の計算・表示まで実装済み。単一平面/Joint Set の完全分割と各片の Chunk 操作も実装済み。曲面・任意角度の全幅部分溝も限定実装した。有限長や交差する複数切断は今後の設計。工程は [実装計画](../plan/plan.md) を参照。
...
パッチ基底で母岩を投影し、U 全幅が母岩を覆い、+V 端が外面の外まで伸びることを求める。終端は従来どおり `extentV - min(depth, 2*extentV)*persistence`。開口の両側に岩があり、終端が -V 側の外面より手前にあることを確認する。

法線方向 ±aperture/2 と V 方向の終端の3平面で `SplitByPlanes` を使って区分する。溝に当たるセルを除き、残存セルの内部区切り面も除去する。残すのは元の外皮、+/- 法線方向の両壁、終端である。外接最大幅の1e-6を許容誤差とし、その半分以内の共有頂点を近傍バケットで結合する。全三角形の閉包・1連結体・正体積、残存体積と除去体積の和（相対1e-4）、両壁/終端の存在を確認し、失敗時は診断して途中結果を破棄する。

この方法は全幅溝に限定し、母岩の連結性で未破断部を確認する。曲面の Bridge 厚さ・面積と断面ガイドは算出しない。既存の矩形断面を曲面へ流用して誤った計測値を表示しない。有限長・交差亀裂には側端面・複数ループ・接続の追加検証が必要。均等 voxel/SDF への置換や汎用自己交差修復は行わない。
