...
更新日時: 2026-09-20 22:49

## 位置づけ

[v2 原仕様](../rock_generator_spec_v2.md) を既存実装へ接続するための設計案。Box の CPU メッシュ・評価・描画変換と有限亀裂パッチのガイド表示は実装済み。軸に沿う単一 Box の部分切断と Bridge の計算・表示まで実装済み。任意角度・複数切断・完全破断は今後の設計。工程は [実装計画](../plan/plan.md) を参照。
