...
Piece Select を選ぶとその入力と選択結果を自動プレビューし、選択を外すと元のプレビューへ戻す。選択された片は面を隠してオレンジのワイヤーフレームで表示する。黄色は割合・減衰の適用前の候補のうち今回は選ばれなかった片、グレーは対象外。ピースの境界線と層別の選択数・棒グラフを表示する。

「削除後」は選択片を隠す比較表示で、実際の削除は Piece Filter で行う。「板を離して表示」はローカルY方向に層を離して内部を見せる。比較表示と間隔は表示専用で、出力・保存形状へ影響しない。手動選択編集は「選択状態」で行う。手動編集中のみ、クリックで選択を解除できるように選択片の面も表示する。編集を終了すると線だけへ戻る。

### 生成と分割

- **Layered Boxes**：枚数、寸法（Yが板厚）、隙間、厚さ・広さのばらつき、面内のずれ、全体の位置と向き、Seed。位置と向きはビューポートのギズモ（Wで移動、Eで回転）でも変えられる。下から層番号0、1…を付ける。板は閉じた独立メッシュで、平行を保つ。
...
2026-09-26追記：同じ板の隣接関係・共有面積から反復的に選択する **Peel** を追加した。[侵食の仕様](piece-erosion.md)。以下のRimの制約は従来モードについてのもの。

- 板は1〜32枚。Scatter の点数は各2〜512点、入力集合全体で最大1024点。分割結果は最大1024片・25万三角形。1枚あたり512点は子IDの間隔で決まり、保存済みのIDを保つため変えない。
- Rim は**元の側縁**を追跡する。新しい外周へ進む場合はPeelを使う。Peelでは上下の板の重なり面積も支持として使う。
