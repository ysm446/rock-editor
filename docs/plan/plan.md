...
P3 の部分切断と Rock Bridge を、軸に沿う単一 Box に限定して実装・検証した。次は P4。

1. `Fracture` ノードで完全分割を明示的に指定し、部分切断の状態とは区別する。最初の入力範囲と Crack からの移行規則を決める。
2. 平面による分割と切断面の生成を実装し、2つの閉じたメッシュ、外向き法線、分割前後の体積を検証する。
3. Chunk の ID・親・状態と分割面の接続情報を保持する。Locked は物理計算せず明示フラグにする。
4. 色分け・個別選択・移動・回転を接続し、設定と Chunk 参照の保存復元、Undo/Redo を検証する。
