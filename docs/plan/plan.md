...
P4 は単一平面・単純断面の2片分割、Chunk の個別操作まで実装した。最初のプロトタイプの限定範囲が揃ったため、次は P5 の母岩形状と複数節理。外観は [参考写真との対応](../reference/visual-target.md) で比較する。

1. Sphere / Ellipsoid / RoundedBox の閉じた CPU メッシュと形状設定を追加し、分割の入力として検証する。
2. 弱い BaseNoise と seed の再現性を検証する。面の反転・自己交差を避ける制約を決める。
3. Joint Set の方向・間隔と2～3系統の生成、複数切断ループ・多片分割を段階的に拡張する。
4. 参考写真の主稜線と割れ目の方向を無地で比較し、細かい分岐亀裂・剥離は別途必要量を判断する。
