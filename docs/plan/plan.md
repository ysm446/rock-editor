...
P0/P1 の実装と Release での表示・保存復元まで完了した（検証の制限は [進捗](progress.md)）。次は P2。

1. CrackPatch の座標系、中心・法線・接線、有限範囲、深さ、persistence、aperture の設定と保存形式を決める。
2. Box に重ねて候補面と有限範囲・深さを半透明表示する。
3. パッチの移動・回転、設定変更、Undo/Redo、保存復元を接続する。
4. 範囲・向き・深さの検証を追加する。この段階では表示のみで、実際の部分切断と Rock Bridge は P3 で作る。
