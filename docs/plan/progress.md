...
P4 の単一平面分割と2片の Chunk 操作を実装した。Box と部分切断済み Box から、単純断面で2つの閉じた片を作れる。次は [計画](plan.md) の P5：母岩形状と節理。写真に対する見た目の目標は [外観目標](../reference/visual-target.md) に記録。

| 区分 | 現在の状態 |
| --- | --- |
| アプリ基盤 | DX12 / ImGui、モデル表示、グラフ編集、素材・アセット・保存基盤あり |
| 岩生成 | Base Rock の Box、寸法 X/Y/Z、Seed の編集に対応。有限亀裂の表示と、軸に沿う単一の部分切断・Rock Bridge に対応。単一平面による完全分割と2片の操作に対応 |
...
| 次の作業 | P5：母岩の形状追加、弱いノイズ、Joint Set、複数分割 |
