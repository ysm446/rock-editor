...
更新日時: 2026-09-21 00:19

## 現在地

P5 前半の母岩4形状と弱い BaseNoise を実装した。曲面・ノイズ付き母岩も Fracture の入力にできる。Joint Set の有限パッチ列と複数方向のガイド表示も追加した。Joint Set を無限平面として使う多片の完全分割・個別操作まで追加した。曲面・斜め方向の全幅部分溝も追加した。次は [計画](plan.md) の有限長/交差パッチの部分切断と Bridge 計測。写真に対する見た目の目標は [外観目標](../reference/visual-target.md) に記録。

| 区分 | 現在の状態 |
| --- | --- |
| アプリ基盤 | DX12 / ImGui、モデル表示、グラフ編集、素材・アセット・保存基盤あり |
| 岩生成 | Base Rock の Box / RoundedBox / Sphere / Ellipsoid、丸み、分割数、弱いノイズと Seed の編集に対応。有限亀裂と Joint Set の複数方向ガイド表示、Box の軸に沿う部分切断・Rock Bridge 計測と、曲面の全幅部分溝に対応。単一平面/Joint Set による完全分割と各片の操作に対応 |
...
| 次の作業 | P5 残り：有限長/交差部分亀裂、曲面の Bridge 計測、外観比較 |
...
- 完全分割の穴・複数ループ対応。部分切断の任意角度・一般メッシュ・複数交差対応。
