...
Rock Editor は、母岩の割れやすい方向、亀裂の深さ、未破断部、完全分離した岩塊をノードで組み立てる Windows 向け岩生成エディタ。現在は Box 生成、有限パッチ表示、軸に沿った単一の部分切断と Rock Bridge に対応。単一平面による完全分割と2片の Chunk 選択・移動・回転にも対応。対応範囲と制約は進捗を参照する。
