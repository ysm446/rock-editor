...
- Joint Set ノードを追加。方向・間隔・本数・位置/角度ばらつき・Seed と有限範囲を設定し、パッチ列をガイド表示する。複数系統の直列接続・合流、保存/読込、Undo/Redo に対応。実分割は後続。
...
- 部分切断は引き続きノイズなしの Box 専用。Joint Set による複数系統の実分割は次の作業。
